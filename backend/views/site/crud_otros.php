...
// PROCESADOR DEL CRUD POST PARA OMAR (CON RETORNO Y MANEJO DE ACTUALIZACIONES)
if (Yii::$app->request->isPost) {
    $post = Yii::$app->request->post();
    if (isset($post['action'])) {
        if ($post['action'] === 'create') {
            $firebase->createReporte('Otros', $post['nivel'], $post['tipo_accion']);
            return Yii::$app->controller->refresh();
        } elseif ($post['action'] === 'update') {
            $firebase->updateReporte($post['id'], 'Otros', $post['nivel'], $post['tipo_accion']);
            return Yii::$app->controller->refresh();
...
// OBTENER Y FILTRAR SOLO REPORTE DE OTROS
$todos = $firebase->getReportes();
$reportesFiltrados = [];
foreach ($todos as $r) {
    if ($r['contenedor'] === 'Otros') {
        $reportesFiltrados[] = $r;
    }
}
?>
<div class="container-fluid p-0">
    <div class="col-md-10 offset-md-2 bg-light min-vh-100 p-4" style="padding-top: 80px !important;">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="text-warning fw-bold m-0 text-dark">🗑️ Gestión de Residuos Generales (Otros)</h2>
            ...
            <div class="col-lg-4 mb-4">
                ...
                <div id="panel-crear" class="card border-0 shadow-sm">
                    ...
                    <div class="card-body p-4">
                        <?= Html::beginForm(['site/crud-otros'], 'post') ?>
                            <input type="hidden" name="action" value="create">
                            <div class="mb-3">
                                <label class="form-label text-muted small fw-bold">Nivel de Llenado (%)</label>
                                <input type="number" name="nivel" class="form-control" min="0" max="100" placeholder="Ej. 10" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <button type="submit" class="btn btn-warning text-dark w-100 fw-bold shadow-sm">Guardar en Firestore</button>
                        <?= Html::endForm() ?>
                    </div>
                </div>
            ...
            <div class="col-lg-8 mb-4">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-dark text-white py-3">
                        <h5 class="mb-0 fw-bold fs-6">Historial Específico (Otros Logs)</h5>
                    </div>
                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    <?php if (empty($reportesFiltrados)): ?>
                                        <tr>
                                            <td colspan="4" class="text-center text-muted py-4">No hay logs de Otros en la base de datos.</td>
                                        </tr>
                                    <?php else: ?>
                                            ...
                                            <tr id="fila-<?= Html::encode($log['id']) ?>">
                                                ...
                                                <td class="text-center px-4">
                                                    <div class="d-flex justify-content-center gap-2">
                                                        ...
                                                        <?= Html::beginForm(['site/crud-otros'], 'post', ['class' => 'm-0', 'onsubmit' => 'return confirm("¿Seguro que quieres borrar este registro de Firestore?");']) ?>
