...
$this->title = 'EcoSort - Módulo Plástico';

$firebase = new \backend\components\FirebaseService();

// PROCESADOR DEL CRUD POST PARA JAFET (CON RETORNO Y MANEJO DE ACTUALIZACIONES)
if (Yii::$app->request->isPost) {
    $post = Yii::$app->request->post();
    if (isset($post['action'])) {
        if ($post['action'] === 'create') {
            $firebase->createReporte('Plástico', $post['nivel'], $post['tipo_accion']);
            return Yii::$app->controller->refresh();
        } elseif ($post['action'] === 'update') {
            $firebase->updateReporte($post['id'], 'Plástico', $post['nivel'], $post['tipo_accion']);
            return Yii::$app->controller->refresh();
...
// OBTENER Y FILTRAR SOLO REPORTE DE PLÁSTICO
...
<div class="container-fluid p-0">
    <div class="col-md-10 offset-md-2 bg-light min-vh-100 p-4" style="padding-top: 80px !important;">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="text-success fw-bold m-0">🥤 Gestión de Residuos de Plástico</h2>
                <p class="text-muted m-0 small">Desarrollador Responsable: <strong>Jafet</strong></p>
            </div>
            ...
            <div class="col-lg-4 mb-4">
                ...
                <div id="panel-crear" class="card border-0 shadow-sm">
                    ...
                    <div class="card-body p-4">
                        <?= Html::beginForm(['site/crud-plastico'], 'post') ?>
                            <input type="hidden" name="action" value="create">
                            <div class="mb-3">
                                <label class="form-label text-muted small fw-bold">Nivel de Llenado (%)</label>
                                <input type="number" name="nivel" class="form-control" min="0" max="100" placeholder="Ej. 45" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <button type="submit" class="btn btn-success w-100 fw-bold shadow-sm">Guardar en Firestore</button>
                        <?= Html::endForm() ?>
                    </div>
                </div>
            ...
            <div class="col-lg-8 mb-4">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-dark text-white py-3">
                        <h5 class="mb-0 fw-bold fs-6">Historial Específico (Plástico Logs)</h5>
                    </div>
                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    <?php if (empty($reportesFiltrados)): ?>
                                        <tr>
                                            <td colspan="4" class="text-center text-muted py-4">No hay logs de Plástico en la base de datos.</td>
                                        </tr>
                                    <?php else: ?>
                                            ...
                                            <tr id="fila-<?= Html::encode($log['id']) ?>">
                                                ...
                                                <td class="text-center px-4">
                                                    <div class="d-flex justify-content-center gap-2">
                                                        ...
                                                        <?= Html::beginForm(['site/crud-plastico'], 'post', ['class' => 'm-0', 'onsubmit' => 'return confirm("¿Seguro que quieres borrar este registro de Firestore?");']) ?>
